    ...
    function categoryMainContent(cat,v){
        ...
        $(v).each((i,value)=>{
            ...
            <div class="news-list-item">
                ...
                <div class="row">
                    <p class="date">
                        <img src="./imgs/calendar.svg" alt="kalendar">${value.date}
                    </p>
                    <p class="date">Author: ${value.author}</p>
                    <p class="date">${value.comments} Comments</p>
                </div>
                <div class="row-grid">
                    <img src="${value.img}" alt="article img">
                    <p>${value.text}</p>
                    <button class="btn-news"><a href="./single.html">Read article</a></button>
                </div>
            </div>
            <hr class="line"/>`;
        })
     ...
     $('.categories li a').click(()=>{
        ...
        .done(function() {
            console.log( "Uspješno učitani podaci iz data.json!");
        })
        .fail(function(err) {
...
if(cat=='news'){
    ...
    .done(function() {
        console.log( "Uspješno učitani podaci iz data.json za kategoriju news!" );
    })
    .fail(function(err) {
        console.log( `error: ${err.status}`);
    })
    ...
    function createComment(name,email,text,date,hours) {
        htmlComment=`
        <div class="row pt-45 m-50-30-0-30">
            <img src="./imgs/user1.png" alt="user image">
            <span>
                <div class="row space-between p-0">
                    <div class="row p-0">
                        <p class="user p-0">${name}</p>
        ...
        if((name!='')&&(email!='')&&text!=''){
            ...
            $.getJSON("../data/data.json",(obj)=>{
                $.each(obj, (key,val)=>{
                    if(key=='comments'){
                        console.log(key,val);
                        val.push({"name":user,"email":email,"text":text,"date":date,"hours":hours});
                    } 
                })
            })//novo
            $('#comments h2:last-of-type').before(createComment(user,email,text,date,hours));     
        }
        else{
